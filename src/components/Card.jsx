function Card() {
    const items = [
        { 
            title: "Guiados por Propósito", 
            description: "Acreditamos que a biotecnologia de precisão pode melhorar drasticamente a experiência de prolongar a vitalidade humana e, quando criada com rigor científico, intenção e excelência, impacta vidas em escala.",
        ...
        { 
            ...
            description: "Vemos uma necessidade clínica e desenvolvemos uma solução. Nossas terapias genéticas proprietárias estão disponíveis para integração com parceiros clínicos e de pesquisa de ponta pelo mundo.",
            image: `<svg width="100%" height="100%" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        { 
            title: "Alianças Biotecnológicas", 
            description: "Fizemos parcerias com centros de pesquisa globais, laboratórios de IA e clínicas de longevidade de luxo para criar tratamentos celulares inovadores sob medida.",
            image: `<svg width="100%" height="100%" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        { 
            title: "Rigor Científico & Bioética", 
            description: "Não apenas especialistas em bioinformática. Cientistas dedicados à longevidade ativa. Somos capazes de navegar tanto por rígidos protocolos regulatórios quanto pela complexidade molecular.",
            image: `<svg width="100%" height="100%" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
        <div className="card-container flex flex-col gap-6 mt-10 w-full">
            {items.map((item, index) => {
                return (
                    <div key={index} className="card w-full md:w-[35vw] flex flex-col sm:flex-row items-start sm:items-center gap-6
                    border border-current/15 rounded-3xl py-6 px-6 sm:py-8 sm:px-8 bg-white/[0.01]">
                        <div className="rightdata w-14 h-14 sm:w-16 sm:h-16 flex-shrink-0 text-current opacity-80"
                            dangerouslySetInnerHTML={{ __html: item.image }}/>
                        <div className="font-[Sansita] leftdata flex-grow">
                            <h1 className="text-xl md:text-[2.5vh] font-bold mb-2 whitespace-normal leading-tight">{item.title}</h1>
                            <p className="text-sm md:text-[2vh] opacity-85 leading-relaxed font-medium">{item.description}</p>
                        </div>                       
                    </div>
                )
            })} 
